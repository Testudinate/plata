...
---------------------------------------------------------------------------
-- ОБЯЗАТЕЛЬНО ПОСЛЕ КАЖДОГО CREATE OR REPLACE.
--
-- CREATE OR REPLACE удаляет объект вместе с ГРАНТАМИ и создаёт новый. Роль
-- LLM_AGENT_RO теряет USAGE, а сервис на VPS начинает получать 399513 —
-- «agent does not exist or access is not authorized». Сообщение уводит в
-- сторону: выглядит как отсутствие объекта или ошибка аутентификации, хотя
-- объект на месте и ключ подошёл.
--
-- Это уже стоило одного полностью провалившегося прогона регресса: 16 вопросов
-- подряд вернули пустые ответы, а в трейсах это выглядело как «0 токенов,
-- модель unknown» — то есть как поломка трейсинга, а не прав.
---------------------------------------------------------------------------

GRANT USAGE ON AGENT RISK_GOV.AGENT.RISK_COPILOT TO ROLE LLM_AGENT_RO;

---------------------------------------------------------------------------
-- ЭКОНОМИЯ ШАГОВ (правка 13.08 после замера расхода).
--
-- Замер одного вопроса: 129 101 prompt-токенов, 2 333 completion, $0.85.
-- В tools_used при этом: cor_metrics, затем system_execute_sql ЧЕТЫРЕ раза —
-- агент прогонял почти один и тот же запрос к семантической вью, перепроверяя
-- сам себя. Каждый вызов инструмента заново тащит весь контекст в промпт,
-- поэтому шесть шагов стоят втрое дороже двух при том же ответе.
--
-- Отсюда в инструкции оркестрации появился блок ECONOMY OF STEPS с потолком
-- в четыре вызова и прямым запретом перепроверять уже полученное число.
-- Экономия здесь не про деньги как таковые: eval из 16 вопросов стоил $8 за
-- прогон, то есть ночной регресс обходился бы в $240 в месяц.
---------------------------------------------------------------------------
