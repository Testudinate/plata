...
-- Чтение результатов DQ-мониторинга (SNOWFLAKE.LOCAL.DATA_QUALITY_MONITORING_RESULTS)
-- отдельного гранта НЕ требует: роль SNOWFLAKE.DATA_METRIC_USER уже унаследована
-- ролью HERMES_MCP_ROLE, доступ к вью проверен запросом. Роли
-- SNOWFLAKE.DATA_QUALITY_MONITORING_VIEWER в этом аккаунте не существует —
-- в списке SHOW DATABASE ROLES IN DATABASE SNOWFLAKE её нет.
